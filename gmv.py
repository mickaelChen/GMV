...
opt.nIteration = 50001        # number of training iterations
...
opt.checkpointFreq = 500      # frequency of checkpoints
...
zContent = torch.FloatTensor(opt.batchSize, opt.sizeZ, 1, 1).normal_()
...
zContent_test = torch.FloatTensor(batch_test, nContents, 1, opt.sizeZ).normal_().repeat(1,1,nViews,1).view(-1, opt.sizeZ, 1, 1)
zView_test = torch.FloatTensor(batch_test, 1, nViews, opt.sizeZ).normal_().repeat(1,nContents,1,1).view(-1, opt.sizeZ, 1, 1)
...
if opt.cuda:
    ...
    zContent = zContent.cuda()
    zView1 = zView1.cuda()
    zView2 = zView2.cuda()
    labelPos = labelPos.cuda()
    labelNeg = labelNeg.cuda()
    zContent_test = zContent_test.cuda()
...
while iteration <= opt.nIteration:
    ...
    for x1_cpu, x2_cpu, _ in tqdm(dataloader):
        ...
        zContent.resize_(x1_cpu.size(0), opt.sizeZ, 1, 1).normal_()
        ...
        netG.zero_grad()
        x1_generated = F.tanh(netG(Variable(torch.cat((zContent,zView1),1))))
        x2_generated = F.tanh(netG(Variable(torch.cat((zContent,zView2),1))))
    ...
    if (epoch) % opt.checkpointFreq == 0:
        netG.eval()
        x_test = F.tanh(netG(Variable(torch.cat((zContent_test, zView_test),1), volatile=True)))
        vutils.save_image(x_test.data, "%d.png" % epoch, nrow=nViews, normalize=True, range=(-1,1))
